...
Intent process_move_j(Tokens* tokens, Handedness current_hand) {
	...
	float angle_0 = 0.0;
	float angle_1 = 0.0;
	if (calculate_scara_ik(x_pos, y_pos, &angle_0, &angle_1, current_hand)) {
		Command move_command = rotate_joint(angle_0, angle_1);
		Commands out_commands = {
			.length = 1,
			.commands = &move_command
		};
		output.commands = out_commands;
	}	else if (calculate_scara_ik(x_pos, y_pos, &angle_0, &angle_1, opposite_hand(current_hand))) {
		Command move_command = rotate_joint(angle_0, angle_1);
		Commands out_commands = {
			.length = 1,
			.commands = &move_command
